...
- **Application** (admissions, A1) — pre-Student pipeline entity, converts into Student + Enrollment on acceptance. Includes a `dateOfBirth` field so Admissions can determine minor status at intake (feeds ConsentRecord below).

### Compliance & consent (added per Somalia DPA Act governance decisions, 2026-08-17)
- **RetentionClass** — a defined category (e.g. "academic record — retain permanently," "applicant data — retain 2 years post-decision," "session/login log — retain 90 days") with its legal basis, owned/approved by Registrar + Legal/Compliance. Referenced by a `retentionClassId` field on every entity holding personal data (Student, Application, Mark, AttendanceRecord, AuditLog, etc.) so retention/deletion tooling has a single place to look up "how long does this row live, and why." Not a placeholder field — it's what makes the storage-limitation principle (§13) and the retention-schedule follow-up actually enforceable rather than aspirational.
- **ConsentRecord** — who consented (self or guardian), to what (terms of service, guardian consent for a minor's data processing, a specific transfer/derogation if ever used), when, and a reference to stored proof. Linked to Application/Student. Required before an underage Application can proceed past intake, per the children's-consent requirement in §13.
- **Redaction/erasure support at the field level, not just the row level** — `AcademicRecordEntry`, `Mark`, and `AttendanceRecord` need the ability to have *non-essential* fields (e.g. free-text notes) redacted/anonymized independently of the mandatory academic fields (grade, credits, semester) that must survive an erasure request under retention obligations (§8, §13). This is a design constraint on those tables, not a new table.
...
11. An erasure request touching `AcademicRecordEntry`, `Mark`, or `AttendanceRecord` data under a mandatory retention period (per its `RetentionClass`) is never executed unilaterally by Admissions or IT/System Admin. It must be reviewed and decided by the governance group — **Registrar (records owner) + Legal/Compliance + Data Protection Officer if appointed** — and the decision (approve/deny/partial) recorded in `AuditLog` with reasoning, per the Somalia DPA Act governance decision (§13).
12. Where an erasure request is denied because of a retention obligation, the system must still support **partial redaction** of non-essential fields (e.g. free-text comments, optional contact details) on the affected record while preserving the mandatory academic fields (grade, credits, semester) intact — never a blanket "erasure denied, nothing changes" response when partial redaction is possible.
13. An underage `Application` (per its `dateOfBirth`) cannot progress past intake without a corresponding guardian `ConsentRecord` on file — enforced at the workflow level, not left to Admissions staff remembering to check.
...
- **Data subject rights** the system must be able to service: access, rectification, erasure, restriction of processing, data portability, objection, and rights relating to automated decisions, plus a statutory right to complain about unlawful/unfair/abusive processing (Art. 26). Concretely, this means:
  ...
  - **Erasure ("right to be forgotten")** directly conflicts with the academic-record retention rule in §8 (rule 6: "deleting a user never deletes academic record rows"). The Act's erasure right is not absolute — it carries exemptions for data an institution is legally/regulatorily obligated to retain (accreditation records, transcripts, audit trails). **Governance decision (2026-08-17):** erasure applies fully to data no longer needed for its original purpose and not covered by a retention obligation (e.g. a rejected applicant's data past its retention window, optional marketing/contact preferences). For core academic records under retention obligation, full erasure is denied until the retention period expires, but **partial redaction/anonymization of non-essential fields is still honored** (§8, rule 12) rather than a flat refusal. **Approval authority:** a governance group of Registrar (records owner) + Legal/Compliance + Data Protection Officer (if appointed) decides every erasure request touching academic records — Admissions and IT/System Admin execute the decision but never decide it alone (§8, rule 11).
...
- **Children's consent.** The DPA treats under-18s as children requiring parent/guardian consent for personal-data processing. **Governance decision (2026-08-17):** the Admissions Office owns confirming underage status at intake (via `dateOfBirth` on Application) and capturing guardian consent with stored proof before processing proceeds (§7's `ConsentRecord`, enforced by §8 rule 13); Legal/Compliance and the DPO (if appointed) own reviewing the consent language and its retention period. Still open: whether your university actually admits any students under 18 — flagged in the decision log for Admissions to confirm.
